...
public struct ScribbleFillGenerator {
    ...
    public static func generate(for path: CGPath, options: Options) -> [OperationSet] {
        measurePerformance(PathOpsSignpost.scribbleFill, log: SketchyPerformanceLog.pathOps, metadata: "tightness=\(options.scribbleTightness)") {
    ...
    private static func findRayIntersections(
        ...
        #if SKETCHY_PERFORMANCE_INSTRUMENTATION || DEBUG
        let signpostID = OSSignpostID(log: SketchyPerformanceLog.pathOps)
        os_signpost(.begin, log: SketchyPerformanceLog.pathOps, name: PathOpsSignpost.rayIntersection, signpostID: signpostID)
        defer { os_signpost(.end, log: SketchyPerformanceLog.pathOps, name: PathOpsSignpost.rayIntersection, signpostID: signpostID) }
        #endif
    ...
    private static func removeDuplicatePoints(_ points: [CGPoint], tolerance: CGFloat) -> [CGPoint] {
        #if SKETCHY_PERFORMANCE_INSTRUMENTATION || DEBUG
        let signpostID = OSSignpostID(log: SketchyPerformanceLog.pathOps)
        os_signpost(.begin, log: SketchyPerformanceLog.pathOps, name: PathOpsSignpost.duplicateRemoval, signpostID: signpostID, "points=%d", points.count)
        defer { os_signpost(.end, log: SketchyPerformanceLog.pathOps, name: PathOpsSignpost.duplicateRemoval, signpostID: signpostID) }
        #endif
