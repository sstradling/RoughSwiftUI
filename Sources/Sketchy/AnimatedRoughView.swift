...
public struct AnimatedRoughView: View {
    ...
    private func computeAllFrames(for size: CGSize) {
        ...
        let baseCommands = measurePerformance(AnimationSignpost.frameRender, log: SketchyPerformanceLog.animation, metadata: "generate base") {
        ...
        varianceQueue.async {
            let cache = measurePerformance(AnimationSignpost.precompute, log: SketchyPerformanceLog.animation, metadata: "steps=\(animConfig.steps)") {
